...
if [ "$API" = "Hash" ]; then
    ...
    do
        ...
        mkdir -p $RES
        # Static Library
        test_execution ar rcvs $RES/libDeviceDetectionHashEngine-$OS-$ARCH.a obj/*.o
    
        # SWIG
        test_execution $CXX_CMD $M $GXXARGS_WRAPPER $SCRIPTPATH/Java_Hash_Engine.cpp -o obj/Java_Hash_Engine.o -I $JAVA_HOME/include -I $JAVA_HOME/include/linux -I $JAVA_HOME/include/darwin

        # Shared Library
        test_execution $CXX_CMD $M $LDARGS obj/Java_Hash_Engine.o -o $RES/DeviceDetectionHashEngine-$OS-$ARCH.so -L$RES -lDeviceDetectionHashEngine-$OS-$ARCH
    done
fi
