...
if [ "$API" = "Pattern" ]; then
  SRCOUT=$SCRIPTPATH/device-detection.pattern.engine.on-premise/src/main/java/fiftyone/devicedetection/pattern/engine/onpremise
  RES=$SCRIPTPATH/device-detection.pattern.engine.on-premise/src/main/resources
fi
...
#    if [ "$API" = "Pattern" ]; then
#        swig -c++ -java -package fiftyone.devicedetection.pattern.engine.onpremise.interop.swig -outdir $SRCOUT/interop/swig -o $SCRIPTPATH/Java_Pattern_Engine.cpp $SCRIPTPATH/device-detection.pattern.engine.on-premise/pattern_java.i
#    fi
#    if [ "$API" = "Hash" ]; then
...
if [ "$API" = "Pattern" ]; then
    ...
    do
        ...
        # SWIG
        g++ $M $GXXARGS $SCRIPTPATH/Java_Pattern_Engine.cpp -o obj/Java_Pattern_Engine.o -I $JAVA_HOME/include -I $JAVA_HOME/include/linux -I $JAVA_HOME/include/darwin
        ...
        # Shared Library
        g++ $M $LDARGS obj/*.o -o $RES/DeviceDetectionPatternEngine-$OS-$ARCH.so
    done
fi
