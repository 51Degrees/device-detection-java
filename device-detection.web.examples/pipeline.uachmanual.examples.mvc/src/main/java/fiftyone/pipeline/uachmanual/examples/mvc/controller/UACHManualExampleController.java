...
public class UACHManualExampleController {
    ...
    public String get(ModelMap model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        AspectPropertyValue<String> unknownPropertyValue = new AspectPropertyValueDefault<>();
        unknownPropertyValue.setNoValueMessage("Property is not present in the Lite data file, need Enterprise data file");
        try {
            model.addAttribute("hardwareVendor", device.getHardwareVendor());
        } catch (Exception e) {
            model.addAttribute("hardwareVendor", unknownPropertyValue);
        }
        try {
            model.addAttribute("hardwareName", device.getHardwareName());
        } catch (Exception e) {
            model.addAttribute("hardwareName", unknownPropertyValue);
        }
        try {
            model.addAttribute("deviceType", device.getDeviceType());
        } catch (Exception e) {
            model.addAttribute("deviceType", unknownPropertyValue);
        }
    	...
    	if (configFile.getAbsolutePath().contains("51Degrees-Hash") ||
    		 ...
    		 engine = flowData.getPipeline().getElement(CloudRequestEngine.class);
