...
public class UAClientHintsExample extends HttpServlet {
	...
	public static void main(String[] args) throws LifecycleException {
		EmbedTomcat.runWebApp("device-detection.web.examples/pipeline.uach.examples.servlet/src/main/webapp",
		"device-detection.web.examples/pipeline.uach.examples.servlet/target",
				8081);
	}
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
			...
			out.println("<style>body {font-family: sans-serif; margin: 2em}</style>");
        	out.println("</head>");
        	out.println("<body>");
        	...
        	if(data.getPipeline().getElement(DeviceDetectionHashEngine.class) != null) {
        		engine = data.getPipeline().getElement(DeviceDetectionHashEngine.class);
        		evidenceKeyfilter = ((DeviceDetectionHashEngine) engine).getEvidenceKeyFilter();
        	}
        	else {
       		 engine = data.getPipeline().getElement(CloudRequestEngine.class);
			...
			DeviceDetectionHashEngine ddhe = data.getPipeline().getElement(DeviceDetectionHashEngine.class);
			...
			// used.
			boolean lite = Objects.nonNull(ddhe) && ddhe.getProperties().size() < 25;

			AspectPropertyValue<String> hardwareVendor = !lite? device.getHardwareVendor() : new AspectPropertyValueDefault<>();
			AspectPropertyValue<List<String>> hardwareName =  !lite? device.getHardwareName() : new AspectPropertyValueDefault<>();
			AspectPropertyValue<String> deviceType = !lite? device.getDeviceType() : new AspectPropertyValueDefault<>();
            ...
            "</br><strong>Detection results:</strong></br>\n" +
           	"    <p>\n" +
           	"        Hardware Vendor: " + (hardwareVendor.hasValue() ? hardwareVendor.getValue() : "<em>Unknown " + hardwareVendor.getNoValueMessage()) + "</em><br />\n" +
           	"        Hardware Name: " + (hardwareName.hasValue() ? stringJoin(hardwareName.getValue(), ",") : "<em>Unknown " + hardwareName.getNoValueMessage()) +"</em><br />\n" +
           	"        Device Type: " + (deviceType.hasValue() ? deviceType.getValue() : "<em>Unknown " + deviceType.getNoValueMessage()) + "</em><br />\n" +
			...
			if (lite) {
				ExampleHelper.doLiteRubric(out, ddhe);
			}
